...
temp_card = card_number.reverse.split('')

numbers = []
temp_card.collect do |x| 
  numbers << x.to_i
end

p numbers

multiply_by_2 = numbers.map.with_index do |value, index|
    if index.odd? 
      value*2
    else
      value  
...
check_greater_9 = multiply_by_2.map do |item|
  if item > 9 
    x = item.to_s
    y = x[0].to_i + x[1].to_i
  else
